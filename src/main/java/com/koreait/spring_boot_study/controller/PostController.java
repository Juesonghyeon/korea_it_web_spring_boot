...
import com.koreait.spring_boot_study.serves.PostService;
...
//    private final PostService postService;
    ...
    public PostController(PostService postService){
        postService.getPost();
        //PostService가 주입되기전 시점에서 사용하게 되면 NPE이 발생할 수도 있다.
        //예를 들어서 생성자에서 바로 쓴다거나 아니면 서비스, 레포지토리 어노테이션을 만들었거나
    }

    //Inversion Of Control => 제어의 역전
    //객체 생성과 제어의 주도권을 개발자가 아닌, 스프링부트가 갖는 것
    //ico container => 스프링부트가 만든 객체들을 담아두고 관리하는 창고
    //필요한 것이 있으면 꺼내서 넣어줌
    //ico 컨테이너에서 해당 객체를 찾아서 자동으로 넣어주니까 우리는 new할 필요가 없다
    //이미 실행될때 ico 컨테이너에 객체들이 생성되서 보관되어있다.

    //의존성 주입, Dependency Injection => DI
    //필요한 객체(의존성)을 직접 만들지 않고, 외부(스프링부트)에서 대신 넣어주는 것
