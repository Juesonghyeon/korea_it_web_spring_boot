...
import com.koreait.spring_boot_study.dto.SignupRequestDto;
...
public class AuthController {
    ...
    @GetMapping("/get/name")
    public String getUsername(@RequestParam(value = "name", defaultValue = "와샌즈") String name, @RequestParam(required = false) Integer age){
        System.out.println(name + age);
        return name + age;
    }
    //안에서 사용하는 변수명과 퀴리스트링의 키값이 다를경우 괄호안에 표기만해주면 됨
    //그리고 기본값도 설정이 가능함
    ...
    //반환 => "검색 조건 - 이름: ***, 이메일: ***"
    @GetMapping("/search")
    public String getUserInfo(@RequestParam(required = false) String name, @RequestParam(defaultValue = "no-email") String email) {
    ...
    public String signup(@RequestBody SignupRequestDto signupRequestDto){
        ...
        return signupRequestDto.toString() + "님의 회원가입이 완료되었습니다.";
    }
